...
терминале). Проверить у любого пользователя: `bash install/diag-git.sh`.
...
```bash
bash install/diag-git.sh     # разбор целиком: что выигрывает и почему
```

Скрипт ничего не меняет — печатает все `git` в `PATH` по порядку, что реально
установлено, сам `PATH` и все правки `PATH` в файлах входа. Запускать **от того же
пользователя и в том же терминале**, где версия старая.

Частые причины:

* терминал не перезапущен — `exec bash -l` или новое окно (`hash -r` для текущего);
* **у пользователя есть `~/.bash_profile`** — bash в логин-шелле читает только
  первый существующий из `~/.bash_profile` → `~/.bash_login` → `~/.profile`,
  остальные игнорирует. У давно заведённых учёток он обычно есть, поэтому
  установщик пишет блок именно в тот файл, который реально читается;
* nvim запущен из меню рабочего стола, а не из терминала — `~/.bashrc` тогда не
  читается вовсе, спасает блок в логин-файле;
* в `/usr/local/bin` остался `git` от старой установки в режиме `system`
  (`sudo rm -f /usr/local/bin/git /usr/local/bin/git-* /usr/local/bin/scalar`).
